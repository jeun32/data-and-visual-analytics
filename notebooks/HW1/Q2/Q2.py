...
def part_1_b_i(connection: Connection, path: str) -> None:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        next(reader)
        cursor = connection.cursor()
        cursor.executemany("INSERT INTO incidents VALUES (?, ?, ?);", reader)
        connection.commit()
    ######################################################################
...
def part_1_b_ii(connection: Connection, path: str) -> None:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        next(reader)
        cursor = connection.cursor()
        cursor.executemany("INSERT INTO details VALUES (?, ?, ?, ?);", reader)
        connection.commit()
    ######################################################################
...
def part_1_b_iii(connection: Connection, path: str) -> None:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        next(reader)
        cursor = connection.cursor()
        cursor.executemany("INSERT INTO outcomes VALUES (?, ?, ?, ?, ?, ?, ?);", reader)
        connection.commit()
    ######################################################################
...
def part_3() -> str:
    ...
    query = """
        SELECT ROUND(
            COUNT(CASE WHEN date BETWEEN '2018-01-01' AND '2020-12-31' THEN 1 END) * 100.0 / COUNT(*),
            2
        )
        FROM incidents;
    """
...
def part_5() -> str:
    ############### EDIT SQL STATEMENT ###################################
    query = """
        SELECT d.detection, COUNT(*) AS count, ROUND(AVG(o.num_ppl_arrested), 2) AS avg_ppl_arrested
        FROM details d
        INNER JOIN outcomes o ON d.report_id = o.report_id
        WHERE o.num_ppl_arrested >= 1
...
def part_8_b() -> str:
    ...
    query = """
        ...
        SELECT report_id, subject FROM details;
    """
    ######################################################################
    return query
